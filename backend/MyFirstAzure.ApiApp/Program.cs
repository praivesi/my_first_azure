...
{
    private readonly IYouTubeVideo _youtube = youtube ?? throw new ArgumentException(nameof(youtube));
    private readonly AzureOpenAIClient _openai = openai ?? throw new ArgumentException(nameof(openai));
    ...
    public async Task<string> SummariseAsync(SummaryRequest req)
    {
        ...
        {
            MaxTokens = int.TryParse(this._config["OpenAI:MaxTokens"], out int maxTokens) ? maxTokens : 3000,
